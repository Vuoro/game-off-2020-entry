...
// Ease-out
export const sinerp = (from, to, time = 0.5) =>
  mix(from, to, sin(time * PI * 0.5));
// Ease-in
export const coserp = (from, to, time = 0.5) =>
  mix(from, to, 1 - cos(time * PI * 0.5));
// Glides to a stop
export const experp = (from, to, time = 0.5) => mix(from, to, time * time);
// Ease-in-out
export const smoothStep = (from, to, time = 0.5) =>
  mix(from, to, time * time * (3 - 2 * time));
...
export const angleBetween = (p1 = [0, 0], p2 = [0, 0]) =>
...
export const angleLerp = (from, to, time) =>
  from + shortAngleDistance(from, to) * time;
...
export const add = (a, b, out = []) => {
  ...
  const dimensions = min(
    aIsArray ? a.length : Infinity,
    bIsArray ? b.length : Infinity
  );
...
export const rotate = (point, angle = 0, around = vec2, out = [...point]) => {
  ...
  out[0] =
    around[0] +
    angleCos * (point[0] - around[0]) -
    angleSin * (point[1] - around[1]);
  out[1] =
    around[1] +
    angleCos * (point[1] - around[1]) +
    angleSin * (point[0] - around[0]);
