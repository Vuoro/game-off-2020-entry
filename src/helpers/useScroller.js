...
export const useScroller = (
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      current.xScrollable =
        (isWindow ? document.body.scrollWidth : scroller.scrollWidth) - width;
      current.yScrollable =
        (isWindow ? document.body.scrollHeight : scroller.scrollHeight) -
        height;
      handleCamera();
    };
...
export const Scroller = ({
  ...
  return (
    <div className="Scroller-scroller" ref={refToUse}>
      {height && width && (
        <div className="Scroller-scrollArea" style={{ height, width }}></div>
      )}
      {children}
    </div>
  );
