...
export const useCommand = (command) => {
  ...
  if (!commands.has(command)) {
    ...
    commands.clear();
    for (const [command, result] of commandArray) {
      commands.set(command, result);
    }
  }
...
const createCommand = (
  ...
  for (
    let i = gl.getProgramParameter(program, gl.ACTIVE_UNIFORMS) - 1;
    i >= 0;
    i--
  ) {
  ...
  for (
    ...
    const hasStaticValue = staticValue !== undefined;
    const value = hasStaticValue ? staticValue : [typeToPlaceholder(type, gl)];

    const divisor = hasStaticValue ? 0 : 1;
    ...
    const usage = hasStaticValue ? gl.STATIC_DRAW : gl.DYNAMIC_DRAW;
    ...
    useEffect(() => {
      if (instance.mounted) {
        for (const key in instanceAttributes) {
          ...
          if (newValue) {
            instanceAttributes[key] = newValue.length
